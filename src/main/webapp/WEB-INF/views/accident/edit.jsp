...
<div class="container">
    <div class="row">
        <ul class="nav">
            <li class="nav-item">
                <h3>
                    Форма редактирования инцидента, заполните все поля
                </h3>
            </li>
        </ul>
    </div>
...
<form class="card-body" action="<c:url value='/save?id=${accident.id}'/>" method='POST'>
    ...
    <div class="form-group" style="margin-left:390px; width: 700px; margin-top: 15px">
        <label>Статус</label>
        <input type="text" class="form-control" name="status" id="status" placeholder="Введите статус"
               value="${accident.status}">
    </div>
    <button style="margin-left:390px; margin-top: 15px" type="submit" class="btn btn-primary" onclick="return validate()">Сохранить</button>
</form>

<script>
    function validate() {
        let fields = [$("#name"), $("#text"), $("#address"), $("#status")];
        let result = true;
        let answer = '';
